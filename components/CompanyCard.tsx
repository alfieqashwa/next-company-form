      ...
      <div className='flex justify-between pb-1 border-b border-gray-300'>
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
          <div className='w-full space-y-10'>
            <h2 className='text-xl font-medium text-center text-red-500'>
              Confirmed ?
            </h2>
            <div className='flex items-center justify-between px-14'>
              <button
                className='px-4 py-2 text-sm text-white transition duration-300 ease-in-out bg-red-500 rounded-md hover:opacity-75 focus:opacity-100 focus:ring-opacity-75 focus:ring-offset-1 focus:ring-2 focus:ring-red-500 focus:outline-none'
                type='button'
                onClick={onClose}>
                Cancel
              </button>
              <button
                className='px-4 py-2 text-sm text-white transition duration-300 ease-in-out bg-green-600 rounded-md hover:opacity-75 focus:opacity-100 focus:ring-opacity-75 focus:ring-offset-1 focus:ring-2 focus:ring-green-600 focus:outline-none'
                type='button'
                onClick={remove}>
                {isLoading ? (
      ...
      <Link href={`/companies/${encodeURIComponent(company.id)}`}>
        <a className='px-1'>
          <div className='py-1'>
            ...
            <h6 className='w-5/6 text-xs tracking-tight capitalize'>
              {company.address}
            </h6>
          </div>
          <div className='py-1'>
