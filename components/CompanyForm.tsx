...
import { FormError } from 'components/ErrorForm';
...
export default function CompanyForm() {
  ...
  const onSubmit = async (data: Prisma.CompanyCreateInput) => {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className='px-2 mb-8 space-y-2'>
          ...
          <>
            <Input
              ...
              placeholder='address'
              type='text'
              formRef={register({ required: true, maxLength: 50 })}
            />
          ...
          <>
            <Input
              ...
              placeholder='revenue'
              type='number'
              formRef={register({ required: true, min: 1, maxLength: 9 })}
            />
          ...
          <div>
            ...
            <div className='flex space-x-5'>
              <div className='space-y-1'>
                <input
                  className='w-16 px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset'
                  placeholder='code'
                  name='code'
                  type='number'
                  ref={register({ required: true, min: 1, maxLength: 3 })}
                />
              ...
              <div className='space-y-1'>
                <input
                  className='w-56 px-2 py-1 border border-gray-300 rounded-md md:w-60 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset'
                  placeholder='phone no'
                  name='phone'
                  type='number'
                  ref={register({ required: true, min: 1, maxLength: 11 })}
                />
                {errors.code?.type === 'required' && (
                  <FormError errorMessage='Code is required' />
                )}
        ...
        <ButtonForm
          ...
          disabledTrueStatus='Creating...'
          disabledFalseStatus='Create'
        />
      </form>
    </div>
  );
